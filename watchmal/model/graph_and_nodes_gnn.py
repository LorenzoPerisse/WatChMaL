...
class ResGateConv_activation_graphvar(torch.nn.Module):
    r""""
    Args :
    ...
    def __init__(
        ...
        self.graph_var_norm = BatchNorm(self.graph_var)
        ...
        for j in range(len(linear_out_features) - 1):
            in_features  = (first_linear_in_features + self.graph_var) if j == 0 else linear_out_features[j - 1]
    ...
    def forward(self, data: torch_geometric.data.Data) -> torch.Tensor:
        x, x_graph, edge_index, batch = data.x, data.x_graph, data.edge_index, data.batch
        ...
        if x_graph.ndimension() == 1:
            x_graph = x_graph.reshape((-1, 1))

        x_graph = self.graph_var_norm(x_graph)

        x_graph = x_graph.squeeze()
        x_graph = x_graph.view(2048, self.graph_var)

        out = torch.cat((xs_gap, xs_gsp, x_graph), dim=1) # out.shape = (batch_size, 2 * conv_in_channels[-1])
